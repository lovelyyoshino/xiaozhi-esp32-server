...
class IntentProvider(IntentProviderBase):
    ...
    def get_intent_system_prompt(self, functions_list: str) -> str:
        ...
        prompt = (
            ...
            "1. 必须使用中文设备名称进行匹配，严禁将中文翻译成英文后再匹配\n"
            "   - 错误示例：用户说'卫生间'时，不要翻译成'toilet'去匹配\n"
            "   - 正确示例：用户说'卫生间'时，直接用'卫生间'在设备列表中查找\n"
            "2. 支持常见同义词自动映射：\n"
            "   - 卫生间 = 厕所 = 洗手间 = WC\n"
            "   - 客厅 = 大厅 = 起居室\n"
            "   - 卧室 = 房间 = 睡房\n"
            ...
            "   - 厨房 = 灶间\n"
            "3. 支持模糊匹配：\n"
            "   - 用户说'卫生间的灯'、'卫生间灯'、'厕所灯'都应匹配到'卫生间,灯'\n"
            "   - 用户说'客厅台灯'应匹配到'客厅,台灯'\n"
            "4. 位置+设备名组合匹配：\n"
            "   - 设备列表格式为'位置,设备名,entity_id'\n"
            "   - 用户输入可能只说位置（如'卫生间'），需要结合上下文推断设备类型\n"
            "   - 用户输入可能只说设备（如'灯'），需要结合上下文推断位置\n\n"
            f"{functions_desc}\n"
            "处理步骤:\n"
            "1. 分析用户输入，确定用户意图\n"
